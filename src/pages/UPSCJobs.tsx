  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // Filter jobs where category or organization includes "UPSC"
        const upscJobs = data.jobs?.filter((job: Job) => 
          job.category?.toLowerCase().includes("upsc") || 
          job.organization?.toLowerCase().includes("upsc")
        ) || [];
        setJobs(upscJobs);
      } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <p className="text-center text-muted-foreground">No UPSC jobs available at the moment.</p>
        ) : (
